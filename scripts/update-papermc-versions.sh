...
# Paper
PAPER_MC_VERSION=$(curl -sS --fail "https://api.papermc.io/v2/projects/paper/" | \
    jq -r '.versions | reverse | .[] | select(startswith("'"${MC_VERSION}"'"))' | head -n1)

PAPER_VERSION=$(curl -sS --fail "https://api.papermc.io/v2/projects/paper/versions/${PAPER_MC_VERSION}/" | \
    jq -r '.builds[-1]')
...
# Waterfall
WATERFALL_VERSION=$(curl -sS --fail "https://api.papermc.io/v2/projects/waterfall/versions/${MC_VERSION}/" | \
    jq -r '.builds[-1]')
